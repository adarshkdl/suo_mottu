...
import { AccusedRow, EMPTY_ACCUSED_ROW } from '@/lib/types';
import Cell from './PersonCell';

export default function AccusedEditor({
  rows,
  onChange,
}: {
  rows: AccusedRow[];
  onChange: (rows: AccusedRow[]) => void;
}) {
  const updateRow = (index: number, patch: Partial<AccusedRow>) => {
    onChange(rows.map((row, i) => (i === index ? { ...row, ...patch } : row)));
  };

  const addRow = () => onChange([...rows, { ...EMPTY_ACCUSED_ROW }]);
  const removeRow = (index: number) => onChange(rows.filter((_, i) => i !== index));

  const setAccusedType = (index: number, accusedType: AccusedRow['accusedType']) => {
    if (accusedType === 'unknown') {
      updateRow(index, { accusedType, name: 'Unknown' });
    } else {
      updateRow(index, { accusedType, name: rows[index].name === 'Unknown' ? '' : rows[index].name });
    }
  };

  return (
    <div className="occ-editor">
      {rows.length === 0 && <div className="occ-empty">No accused added yet</div>}

      {rows.map((row, i) => (
        <div className="occ-card" key={i}>
          <div className="occ-card-header">
            <div className="occ-card-header-left">
              <span className="occ-card-title">Accused {i + 1}{row.accusedType === 'known' && row.name ? ` — ${row.name}` : ''}</span>
              <div className="occ-type-toggle">
                <button
                  type="button"
                  className={`occ-toggle-btn${row.accusedType === 'known' ? ' occ-toggle-btn--active' : ''}`}
                  onClick={() => setAccusedType(i, 'known')}
                >
                  Known
                </button>
                <button
                  type="button"
                  className={`occ-toggle-btn${row.accusedType === 'unknown' ? ' occ-toggle-btn--active' : ''}`}
                  onClick={() => setAccusedType(i, 'unknown')}
                >
                  Unknown
                </button>
              </div>
